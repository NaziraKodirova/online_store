...
from keyboards.client_inline_keyboards import left_right_k
...
async def ad_phone_handler(message: Message, state: FSMContext):
    ...
    try:
        ...
        if x:
            await state.clear()
            await message.answer("sizning reklamangiz muvaffaqiyatli ro`yhatdan o`tkazildi!")
        else:
            await message.answer("Xatolik, iltimos birozdan so`ng qayta uruning...")
    except:
...
async def all_ads_handler(message: Message, state: FSMContext):
    ...
    if all_ads is None:
        await message.answer("Sizda hech qanday reklamalar yo`q!\n")
    elif len(all_ads) == 1:
